...
def validate_reading_unit(record: dict, unit_id: str) -> str | None:
    if record.get("unit_id") != unit_id: return f"Primary unit identifier mismatch for {unit_id}"
    if record.get("status") != "DRAFTED_PENDING_OWNER_REVIEW": return f"Primary unit status mismatch for {unit_id}"
    ...
    if record.get("secondary_comparison_status") != "DEFERRED": return f"Primary unit secondary comparison must remain deferred for {unit_id}"
    required_sections = ["bibliographic_and_witness_control", "narrative_person_and_authorial_attribution",
    ...
    for section in required_sections:
        if not record.get(section): return f"Required primary section {section} missing for {unit_id}"
    ...
    for finding in record["provisional_findings"]:
        if finding.get("evidence_type") != "PROVISIONAL_INFERENCE": return f"Primary provisional finding must be typed in {unit_id}"
    for question in record["standing_unresolved_questions"]:
        if question.get("evidence_type") != "UNRESOLVED_QUESTION": return f"Primary unresolved question must be typed in {unit_id}"
    return None
...
def main() -> int:
    ...
    if documents[ROOT / "corpus/sources/strauss-xenophons-anabasis.yaml"].get("status") != "OWNER_ADOPTED_SECONDARY_SOURCE": return fail("Secondary source owner-adoption status mismatch")
    if documents[ROOT / "corpus/witnesses/strauss-spp-1983.yaml"].get("status") != "OWNER_ADOPTED_SECONDARY_WITNESS": return fail("Secondary witness owner-adoption status mismatch")
    primary_source = documents[ROOT / "corpus/sources/xenophon-anabasis.yaml"]
    if primary_source.get("status") != "OWNER_ADMITTED_PRIMARY_TRANSLATION_SOURCE" or primary_source.get("work", {}).get("author") != "Xenophon" or primary_source.get("edition", {}).get("translator") != "H. G. Dakyns": return fail("Primary source control mismatch")
    primary_witness = documents[ROOT / "corpus/witnesses/gutenberg-1170-dakyns-pdf.yaml"]
    if primary_witness.get("status") != "OWNER_ADMITTED_PRIMARY_TRANSLATION_WITNESS" or primary_witness.get("source_id") != "XEN-SRC-PRI-001": return fail("Primary witness admission mismatch")
    if primary_witness.get("witness", {}).get("page_count") != 168: return fail("Primary witness page count mismatch")
    if primary_witness.get("file_control", {}).get("sha256") != "6a7534d8d80153afc1623803ef129185aa8d3d41be692091f4e105375c65901e": return fail("Primary witness SHA-256 mismatch")

    secondary_review = documents[SECONDARY_OWNER_REVIEW]
    if secondary_review.get("status") != "OWNER_ADOPTED_SECONDARY_RECONSTRUCTION" or secondary_review.get("scope", {}).get("units") != SECONDARY_UNIT_IDS: return fail("Secondary owner review mismatch")
    primary_admission = documents[PRIMARY_ADMISSION]
    if primary_admission.get("status") != "OWNER_ADMITTED_PRIMARY_TRANSLATION_WITNESS": return fail("Primary admission record status mismatch")
    if primary_admission.get("scope", {}).get("initial_unit") != PRIMARY_UNIT_IDS[0]: return fail("Primary admission initial-unit mismatch")
    if primary_admission.get("limits", [])[-1:] != ["Artificial-intelligence self-certification remains prohibited."]: return fail("Primary admission safeguard missing")

    secondary_plan = documents[ROOT / "studies/strauss-xenophons-anabasis/reading-plan.yaml"]
    if secondary_plan.get("status") != "OWNER_ADOPTED_SECONDARY_RECONSTRUCTION": return fail("Secondary reading plan owner-adoption status mismatch")
    drafted_secondary = [u["id"] for u in secondary_plan["reading_units"] if u.get("status") == "DRAFTED_PENDING_OWNER_REVIEW"]
    if drafted_secondary != SECONDARY_UNIT_IDS: return fail("Secondary drafted-unit order mismatch")
    ...
    if unit11.get("narrative_person_and_authorial_attribution", {}).get("xenophon_as_character_present") != "TEXTUALLY_DISPUTED": return fail("Anabasis II.1 Theopompus/Xenophon attribution uncertainty must remain preserved")
    unit12 = documents[PRIMARY_UNIT_PATHS["XEN-PRI-RU-012"]]
    if "TEXTUAL_VARIANT_OBSERVATION" not in {o.get("evidence_type") for o in unit12.get("documentary_observations", [])}: return fail("Anabasis II.2 wolf textual variant must remain documented")
    if not any("fortune proved a better general" in o.get("observation", "") for o in unit12.get("documentary_observations", [])): return fail("Anabasis II.2 narratorial judgment must remain represented")
    unit13 = documents[PRIMARY_UNIT_PATHS["XEN-PRI-RU-013"]]
    if unit13.get("narrative_person_and_authorial_attribution", {}).get("first_person_narrator_present") is not True: return fail("Anabasis II.3 first-person narratorial intervention must remain represented")
    unit14 = documents[PRIMARY_UNIT_PATHS["XEN-PRI-RU-014"]]
    if unit14.get("narrative_person_and_authorial_attribution", {}).get("xenophon_as_character_present") is not True: return fail("Anabasis II.4 named Xenophon appearance must remain represented")
    if "possibly Xenophon" not in unit14.get("narrative_person_and_authorial_attribution", {}).get("textually_disputed_or_conjectural_identification", ""): return fail("Anabasis II.4 conjectural young-man identification must remain preserved")
