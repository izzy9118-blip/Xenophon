...
def main():
    ...
    if manifest.get("version") != "1.69.0" or manifest.get("state") != "MINISTER_ADAPTER_R1_DRAFT_COMPLETE_PENDING_OWNER_REVIEW":
    ...
    expected_defects = ["LIVE_SCHEMA_NONCONFORMANCE", "WITNESS_IDENTIFIER_INCOMPATIBILITY", "FALSE_REPOSITORY_PIN", "PLACEHOLDER_UNVERIFIED_HASHES", "DERIVATION_AND_OPERATIONAL_AUTHORITY_COLLAPSE"]
    if adapter_state.get("corrected_defects") != expected_defects:
    ...
    if preserved.get("registers") != [f"XEN-REGISTER-{i:03d}" for i in range(1, 5)] or preserved.get("guards") != [f"XEN-GUARD-{i:03d}" for i in range(1, 4)]:
        return fail("approved adapter architecture changed")
    ...
    if request.get("repository_pin", {}).get("commit") != "312d5fddf2f6e63ba383e3cfb8a45ba0641a480b" or request.get("repository_pin", {}).get("manifest_version") != "1.69.0":
    ...
    if gates.get("artificial_intelligence_self_certification_prohibited") is not True or gates.get("sanctum_registration_present") is not False or gates.get("assembly_dispatch_authorized") is not False:
        return fail("governance gates changed prematurely")
